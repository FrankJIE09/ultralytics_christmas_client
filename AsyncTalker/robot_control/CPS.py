...
class CPSClient(object):
    ...
    *	@param return: 错误码
    ...
    def cps_moveL(cps_client, target_pose, ucs, speed, acceleration, radius, tcp="TCP", timeout=60):
        """
        封装的函数，用于调用HRIF_MoveL并等待运动完成。

        :param cps_client: CPS客户端实例
        :param target_pose: 目标位姿
        :param ucs: 用户坐标系
        :param speed: 运动速度
        :param acceleration: 加速度
        :param radius: 圆弧半径
        :param tcp: 工具坐标系名称，默认为"TCP"
        :param timeout: 等待运动完成的超时时间（秒）
        :return: True表示成功，False表示失败
        """
        ret = cps_client.HRIF_MoveL(
            0, 0,
            points=target_pose,
            RawACSpoints=target_pose,
            tcp=tcp,
            ucs=ucs,
            speed=speed,
            Acc=acceleration,
